...
try:
    from GhrelinModPy.ghrelinmod import *
except Exception:
    pass
...
class MainFrame(wx.Frame):
    def __init__(self, title, pos, size, rpath, mpath):
        ...
        self.app_path = os.path.dirname(module_dir)

        if self.ostype == "Windows":
            self.respath = os.path.join(self.app_path, "HypoModPy", "Resource")
        else:
            self.respath = os.path.join(self.app_path, "HypoModPy", "Resource")
...
class HypoMain(MainFrame):
    def __init__(self, title, pos, size, respath, mainpath, modname):
        ...
        if modname == "Ghrelin": self.mod = GhrelinMod(self, "ghrelinmod")
