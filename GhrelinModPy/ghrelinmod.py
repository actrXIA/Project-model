...
class GhrelinMod(Mod):
    def __init__(self, mainwin, tag):
        ...
        self.ModLoad()
        print("Ghrelin Trigger-Comparison Model OK")
    ...
    def PlotData(self):
        self.plotbase.AddPlot(
            PlotDat(self.ghrelindata.ghrelin, 0, 1440, 0, 160,
                    "ghrelin", "line", 1, "blue"),
            "ghrelin"
        )

        self.plotbase.AddPlot(
            PlotDat(self.ghrelindata.meal, 0, 1440, 0, 2.0,
                    "meal input", "line", 1, "red"),
            "meal"
        )

        self.plotbase.AddPlot(
            PlotDat(self.ghrelindata.circ_target, 0, 1440, 0, 160,
                    "circadian target", "line", 1, "purple"),
            "circ_target"
        )

        self.plotbase.AddPlot(
            PlotDat(self.ghrelindata.fullness, 0, 1440, 0, 18,
                    "fullness", "line", 1, "green"),
            "fullness"
        )

        self.plotbase.AddPlot(
            PlotDat(self.ghrelindata.insulin, 0, 1440, 0, 4.0,
                    "insulin", "line", 1, "purple"),
            "insulin"
        )

        self.plotbase.AddPlot(
            PlotDat(self.ghrelindata.lightdark, 0, 1440, 0, 1.2,
    ...
    def RunModel(self):
        self.mainwin.SetStatusText("Ghrelin Trigger-Comparison Model Run")
...
class GhrelinBox(ParamBox):
    def __init__(self, mod, tag, title, position, size):
        ...
        self.paramset.AddCon("g_threshold", "G trig lvl", 90.0, 1.0, 1)
        self.paramset.AddCon("dg_threshold", "dG trig", 0.030, 0.005, 3)
        self.paramset.AddCon("refractory", "Pause min", 40, 5, 0)
        self.paramset.AddCon("fullness_cap", "F cap", 14.0, 0.5, 1)
        self.paramset.AddCon("trig_meal_size", "Trig meal", 1.0, 0.1, 1)
...
class GhrelinModel(ModThread):
    ...
    def Model(self):
        ...
        for global_step in range(1, total_steps + 1):
            ...
            above_G = G >= eff_g_threshold
            above_dG = dG_current >= eff_dg_threshold

            can_trigger = (meal_timer <= 0) and (refractory_timer <= 0) and (F < fullness_cap)

            trigger_now = False
            if trigger_mode == 1:
                # Persistent-threshold mode:
                # if ghrelin stays above threshold, a new meal can start whenever
                # the pause/refractory and fullness gates allow it.
                trigger_now = can_trigger and above_G
            elif trigger_mode == 2:
                # Rate-trigger mode kept as an upward crossing of dG.
                trigger_now = can_trigger and (not prev_above_dG) and above_dG

            prev_above_G = above_G
            prev_above_dG = above_dG
